    ...
    async.auto({
      ...
      start: function (cb) {
        ...
        liftAbortTimer = setTimeout(function failedToStart() {
          ...
          // 0. Just a slow Gulp task
          if (sails.hooks.gulp) {
            if (process.env.NODE_ENV === 'production') {
              sails.log.error(
                ' -> Do you have a slow Gulp task?  You are running in production mode where, by default, tasks are configured to minify the JavaScript and CSS/LESS files in your assets/ directory.  Sometimes, these processes can be slow, particularly if you have lots of these types of files.'
              );
            }
            else {
              sails.log.error(
                ' -> Do you have a slow Gulp task, or lots of assets?'
              );
            }
            sails.log.error();
          }
          ...
          if (sails.config.port < 1024) {
            ...
            // Windows users should try to run as administrator
